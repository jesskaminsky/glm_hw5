...
y <- 3 + .1*x.1 + .5*x.2 + rt.scaled(n = 100, df = 4, mean = 0, sd = 5)
...
(a)  Fit a Poisson regression model predicting number of unprotected sex acts from baseline HIV status. Perform predictive simulation to generate 1000 datasets and record both the percent of observations that are equal to 0 and the percentthat are greater than 10 (the third quartile in the observed data) for each. Compare these values to the observed value in the original data.
#USE FUPACTS
...
# y.rep <- rpois(length(risky$bupacts), y.hat)

X <- cbind(rep(1, length(risky$bupacts)), risky$bs_hiv)
sim4a <- sim(model_4a, 1000)
y4a <- array(NA, c(1000, length(risky$bupacts)))
for (s in 1:1000) {
  y4a.hat <- exp(X %*% sim4a@coef[s,])
  y4a[s,] <- rpois(length(risky$bupacts), y4a.hat)
}
